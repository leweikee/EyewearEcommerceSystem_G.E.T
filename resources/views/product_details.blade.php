...
<head>
    ...
    <script src="{{ asset('/resources/js/productDetails.js') }}"></script>
    <link rel="stylesheet" href="{{ asset('/resources/css/default.css') }}">
    <link rel="stylesheet" href="{{ asset('/resources/css/productDetails.css') }}">
</head>
        ...
        <div class="row">
            ...
            <div class="col-md-6">
                <div class="product-details rounded-4" style="border: 1px solid #ddd;">
                    <img src="{{ asset('product/' . $item->image) }}" alt="{{ $item->name }}" class="img-fluid" style="height: 100%">
                </div>
            </div>
            <!-- Display details on the right -->
            <div class="col-md-6">
                <div class="product-details">
                    <h6 class="fw-light font-monospace mb-4">{{ $item->brand->brandName }}</h6>
                    <h2 class="fw-bold mb-4">{{ $item->name }}</h2>
                    
                    <!--if disPrice exists-->
                    @if ($item->disPrice)
                        <h3 class="fw-semibold mb-4" style="color: red;">RM {{ number_format($item->disPrice, 2) }}</h3>
                        <p class="text-decoration-line-through">RM {{ number_format($item->price, 2) }}</p>
                    @else
                        <h3 class="fw-semibold mb-4" style="color: red;">RM {{ number_format($item->price, 2) }}</h3>
                    @endif

                    <!--sunglasses description-->
                    @if ($item->category=="sunglasses")
                        <p class="fw-light mb-0" id="description">{!! nl2br($item->description) !!}</p>
                    <!--contactlens description-->
                    @elseif ($item->category=="contactlens")
                        <p class="fw-light mb-0" id="description">
                            {!! nl2br(e(strstr($item->description, 'Lens Specification', true))) !!}
                        </p>
                        <!-- Display the Lens Specification section as a dropdown if it exists -->
                        @if (strpos($item->description, 'Lens Specification') !== false)
                        ...
                        <div class="collapse fw-light" id="lensSpecificationCollapse">
                            {!! preg_replace('/<br\s*\/?>\s*<br\s*\/?>/i', '<br>', nl2br(e(trim(strstr($item->description, 'Box Contains'))))) !!}
                        </div>
                        @endif
                    ...
                    <form action="{{ url('/add_cart/' . $item->itemID) }}" method="post" class="mt-4" onsubmit="return validateQuantity()">
                        @csrf
                        ...
                        <div class="mt-2 mb-4 rounded-pill" style="width: auto; max-width: 30%; background-color: white; border: 1px solid black; overflow: hidden;">
                            <input type="text" class="form-control text-center" value="{{$item->colour}}" placeholder="{{$item->colour}}" aria-label="Disabled input example" disabled readonly>
                        </div>
                        @endif

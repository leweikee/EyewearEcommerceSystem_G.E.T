...
<html lang="{{ str_replace('_', '-', app()->getLocale()) }}">
  <head>
    ...
    <!-- Include the header -->
    <link rel="stylesheet" href="{{ asset('resources/css/custProfile.css') }}">
    <link rel="stylesheet" href="https://cdn.jsdelivr.net/npm/bootstrap-icons@1.11.3/font/bootstrap-icons.min.css">
    <script src="{{ asset('/resources/js/custProfile.js') }}"></script>
  </head>
  <body>
                    ...
                    <div class="d-flex align-items-center mb-3">
                        ...
                        <div class="ms-3">
                            <div class="d-flex align-items-center mb-3">
                                ...
                                </svg><p class="mb-0 ps-2">Username: {{ $user->name }}</p></div>
                                <div class="d-flex align-items-center mb-3">
                                ...
                                </svg><p class="mb-0 ps-2">Email: {{ $user->email }}</p></div>
                                <div class="d-flex align-items-center mb-3">
                                ...
                                </svg><p class="mb-0 ps-2">Phone number: {{ $user->phoneNum }}</p></div>
                    ...
                    <div class="addresslist ms-3">
                        <table class="table table-striped">
                            ...
                            <tr>
                                ...
                                <div class="d-inline">
                                    <span>{{ $address->recipientName }} | {{ $address->recipientPhoneNum }}</span>
                                    @if($address->is_default == 1)
                                    <span class="badge bg-success">Default Address</span>
                                    @endif
                                    <br>
                                    <span>{{ $address->address }}</span><br>
                                    <span>{{ $address->city }}, {{ $address->state }}, {{ $address->postcode }}</span><br>
                                </div>
                                <!-- @if($address->is_default == 1)
                                ...
                                <div class="d-inline me-3">
                                    <div class="d-flex justify-content-between me-1" style="width: 120.88px;">
                                        <button type="button" class="btn text-primary" onclick="editAddress('{{ $address->addressID }}', {{ json_encode($address) }})"><span class="h6" style="color: #362F27">Edit</span></button>
                                        <!-- editAddress('{{ $address->addressID }}', {{ json_encode($address) }}) -->
                                        @if($address->is_default == 0)
                                        &nbsp;<button type="button" class="btn text-primary" onclick="deleteAddress('{{ $address->addressID }}')"><span class="h6" style="color: #362F27">Delete</span></button>
                                        @endif
                                    </div>
                                    @if($address->is_default==0)
                                    <button type="button" class="btn btn-outline-secondary" onclick="setDefaultAddress('{{ $address->addressID}}', '{{ $address->userID}}')">Set as default</button>
                                    @else
    ...
    <div class="modal" id="editModal" aria-labelledby="editModalLabel" aria-hidden="true">
        <div class="modal-dialog">
            <div class="modal-content">
                ...
                <form method="post" action="{{ url('/updateProfile', $user->id) }}">
                    @csrf
                    <div class="modal-body">
                        ...
                        <div class="mb-3">
                            ...
                            <input type="text" id="phoneNum" name="phoneNum" class="form-control" value="{{ $user->phoneNum }}">
                        </div>
                    </div>
    ...
    <div class="modal" id="addAddress" aria-labelledby="addNewAddress" aria-hidden="true">
        <div class="modal-dialog">
            <div class="modal-content">
                ...
                <form method="post" action="{{ url('/addAddress', $user->id) }}">
                    @csrf
                    <div class="modal-body">
                        <!-- Input fields for editing -->
                        <div class="mb-3">
    ...
    <div class="modal" id="editAddress" aria-labelledby="editAddress" aria-hidden="true">
        <div class="modal-dialog">
            <div class="modal-content">
                ...
                <form method="post" action="{{ url('/editAddress', ['id' => $user->id, 'addressID' => $address->addressID ?? null]) }}">
                    @csrf
                    <div class="modal-body">
                        <!-- Input fields for editing -->
                        <div class="mb-3">
